...
var player_id: int = 2
var player_name: String = "JUGADOR 2"
...
# Colores del jugador
var player_color = Color(0.2, 0.8, 0.2)  # Verde
...
func _setup_health_bar():
	if health_bar:
		...
		# Estilo personalizado para Jugador 2
		var style = StyleBoxFlat.new()
		style.bg_color = Color(0.05, 0.1, 0.05)
...
func _connect_signals():
	...
	if Signals.has_signal("Player2Damage"):
		if not Signals.Player2Damage.is_connected(_on_damage):
			Signals.Player2Damage.connect(_on_damage)
	
	# Conectar señales de notas
	if Signals.has_signal("Player2_note_hit"):
		if not Signals.Player2_note_hit.is_connected(_on_note_hit):
			Signals.Player2_note_hit.connect(_on_note_hit)
	
	if Signals.has_signal("Player2_increment_combo"):
		if not Signals.Player2_increment_combo.is_connected(_on_increment_combo):
			Signals.Player2_increment_combo.connect(_on_increment_combo)
	
	if Signals.has_signal("Player2_reset_combo"):
		if not Signals.Player2_reset_combo.is_connected(_on_reset_combo):
			Signals.Player2_reset_combo.connect(_on_reset_combo)
	
	if Signals.has_signal("Player2_add_score"):
		if not Signals.Player2_add_score.is_connected(_on_add_score):
...
func _on_increment_combo():
	...
	if GameManager:
		GameManager.update_best_combo_player(2, combo_count)
...
func _on_add_score(incr: int):
	total_score += incr
	if GameManager:
		GameManager.current_scores.player2 = total_score
...
func _play_damage_effect():
	if sprite:
		...
		damage_flash_tween.tween_property(sprite, "modulate", Color(2.0, 2.0, 2.0, 1.0), 0.08)
...
func _player_death():
	...
	if GameManager:
		GameManager.perfect_count_p2 = perfect_count
		GameManager.great_count_p2 = great_count
		GameManager.good_count_p2 = good_count
		GameManager.ok_count_p2 = ok_count
		GameManager.miss_count_p2 = miss_count
