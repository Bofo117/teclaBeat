...
# Multiplicadores por combo (se cargan de DifficultyManager)
...
# Spawn delays (se cargan de DifficultyManager)
...
func _ready():
	...
	print("✅ KeyListener2P listo: ", key_name, " (Jugador ", player_id, ") | Spawn aleatorio: ", enable_random_spawn)

func _connect_combo_signals():
	# Desconectar señales previas para evitar duplicados
	if player_id == 1:
		if Signals.Player1_increment_combo.is_connected(_on_combo_incremented):
			Signals.Player1_increment_combo.disconnect(_on_combo_incremented)
		if Signals.Player1_reset_combo.is_connected(_on_combo_reset):
			Signals.Player1_reset_combo.disconnect(_on_combo_reset)
		
		Signals.Player1_increment_combo.connect(_on_combo_incremented)
		Signals.Player1_reset_combo.connect(_on_combo_reset)
	else:
		if Signals.Player2_increment_combo.is_connected(_on_combo_incremented):
			Signals.Player2_increment_combo.disconnect(_on_combo_incremented)
		if Signals.Player2_reset_combo.is_connected(_on_combo_reset):
			Signals.Player2_reset_combo.disconnect(_on_combo_reset)
...
func _load_difficulty_config():
	if DifficultyManager and DifficultyManager.has_method("get_spawn_delays"):
		...
		# Cargar multiplicadores de combo
...
func _setup_random_spawn():
	...
	spawn_timer.start(randf_range(0.5, 2.0))
...
func _on_key_pressed():
	...
	if distance < ok_threshold:
		...
		# FUERA DE RANGO
		print("❌ FUERA DE RANGO - Jugador ", player_id, " - Tecla: ", key_name)
...
func _process_hit(result: Dictionary, key):
	...
	if result["text"] != "MISS":
		...
		print("🎯 J", player_id, " ", result["text"], " | +", result["score"], " pts | Combo: ", current_combo)
	else:
		_handle_miss()
...
func _handle_miss():
	...
	# Emitir daño al jugador específico
	if player_id == 1:
		Signals.Player1Damage.emit(damage_amount)
	else:
		Signals.Player2Damage.emit(damage_amount)
...
func _on_game_over():
	...
	print("🛑 Game Over en ", key_name, " (J", player_id, ")")
...
func _on_game_restart():
	print("🔄 Reinicio en ", key_name, " (J", player_id, ")")
	is_game_active = true
	is_processing_key = false
	...
	if enable_random_spawn:
		...
		spawn_timer.timeout.connect(_on_spawn_timer_timeout)
		add_child(spawn_timer)
		spawn_timer.start(randf_range(current_min_spawn_delay, current_max_spawn_delay))
